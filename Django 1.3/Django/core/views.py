...
from .models import Tour,Genero, Usuario, CarritoItem
...
@login_required
def agregar_al_carrito(request, tour_id):
    tour = get_object_or_404(Tour, id_tour=tour_id)
    ...
    carrito_item, created = CarritoItem.objects.get_or_create(user=usuario, tour=tour)
    if not created:
...
def carrito(request):
    ...
    for item in carrito_items:
        item.subtotal = item.tour.precio * item.cantidad
...
def tour_add(request):
    formTour = TourForm()
    if request.method == "POST":
        nuevo = TourForm(request.POST, request.FILES)
        if nuevo.is_valid():
            nuevo.save()
